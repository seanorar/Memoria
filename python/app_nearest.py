...
from code_m import vis_nearest_rois

proto_val = "prototxt"
model_val = "caffemodel"
img_val = "image"
data_folder_val  = "data_folder"
...
def set_data_folder(label):
    global data_folder_val
    data_folder_val = load_folder(label)
...
def create_img_roi():
    ...
    global proto_val
    global img_val
    vis_nearest_rois(img_val,proto_val, model_val ,data_folder_val, "cpu")
...
proto_buttom = Button(proto_frame,text="select prototxt",
...
model_buttom = Button(model_frame,
                         text="select caffemodel", fg="black",
                         command= lambda : set_model(model_label)).pack(side="left")
model_label.pack()

data_frame = Frame(root)
data_frame.pack()
data_label = Message(data_frame, text = data_folder_val, width=100)
data_buttom = Button(data_frame,
                         text="select data_folder", fg="black",
                         command= lambda : set_data_folder(data_label)).pack(side="left")
data_label.pack()
