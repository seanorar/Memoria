...
def get_all_features(imgs_names, bboxs_list):
    ...
    for i in range(0, 1):
        ...
        aux_list = []
        for bbox in bboxs:
            im_roi = extrac_roi(im, bbox)
            feature = feature_detect(net, im_roi)
            aux_list.append(feature)
        feature_list.append(aux_list)
    return  feature_list

def get_features_from_list(imgs_list_names):
    cfg.TEST.HAS_RPN = True  # Use RPN for proposals
    ...
    for im_name in imgs_list_names:
        ...
        if im !=None:
            if (len(im) > 39 and len(im[0]) > 39):
                ...
                result_name.append(im_name)
        else:
            print "fallo en "+str(im_name)
    return  (feature_list, result_name)
